...
func (u UserController) Register(c *gin.Context) {
	...
	if username == "" || password == "" || confirmPassword == "" || email == "" {
		models.ReturnError(c, 4001, "输入的用户名、密码、确认密码、邮箱其中有空项")
		return
	}
...
func (u UserController) Login(c *gin.Context) {
	...
	token, err := models.GenerateToken(user1.Username)
	if err != nil {
...
func (u UserController) UserDelete(c *gin.Context) {
	...
	//删除用户
	err2 := models.DeleteUserByUsername(user1.Username)
	if err2 != nil {
	...
	//删除用户
	err4 := models.DeleteUserByUsername(user1.Username)
	if err4 != nil {
		models.ReturnError(c, 4014, "删除用户失败")
		return
	}
